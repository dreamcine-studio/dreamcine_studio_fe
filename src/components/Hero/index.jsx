...
<div className=" flex justify-center space-x-4">
    {movies.map((movie, index) => (
        <div key={index} className="max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
            <Link to={`/movies/${movie.id}`}>
                <img className="rounded-t-lg h-96 w-full object-cover" src={movie.poster} alt="" />
           
            <div className="p-5">
                <a href="#">
                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{getGenreName(movie.genre_id)} </h5>
                </a>
